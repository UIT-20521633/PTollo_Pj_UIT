...
//fetchInvitationsAPI: Lấy danh sách thông báo mời từ server về và cập nhật vào Redux store
...
//updateBoardInvitationAPI: Cập nhật trạng thái của thông báo mời từ server về và cập nhật vào Redux store
export const updateBoardInvitationAPI = createAsyncThunk(
  "notifications/updateBoardInvitationAPI",
  async ({ status, invitationId }) => {
    const response = await authorizedAxiosInstance.put(
      `${API_ROOT}/v1/invitations/boards/${invitationId}`,
      { status }
    );
    return response.data;
  }
